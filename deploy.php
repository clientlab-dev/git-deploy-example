...
$json    = json_decode($_REQUEST['payload'], true);
...
// if none of the above match, but a token exists, exit
    ...
    if ($json["ref"] === BRANCH) {
        ...
        if ( is_dir(DIR)) {
            ...
            fputs($file, "DIR: ". DIR . "\n");
            fputs($file, "file_exists(DIR . '.git'): ". print_r(file_exists(DIR . ".git"),true) . "\n");
            fputs($file, "is_dir(DIR): ". print_r(is_dir(DIR),true) . "\n");
        }
    } else{
        fputs($file, "=== ERROR: Pushed branch does not match BRANCH ===\n");
        fputs($file, "json: ".$json["ref"]."\n");
        fputs($file, "BRANCH: ".BRANCH."\n");
        //fputs($file, "POST: ".print_r($_REQUEST,true)."\n");
    }
}
